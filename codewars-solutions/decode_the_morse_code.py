...
def decodeMorse(morseCode):
    ...
    logging.debug('morseCode: ' + str(morseCode))
    processedMorse = [morseWord.split(' ') for morseWord in morseCode.split('   ')]
    logging.debug('processedMorse: ' + str(processedMorse))
    logging.debug('')
    
    for word in processedMorse:
        logging.debug('word: ' + str(word))
        for letter in word:
            logging.debug('letter: ' + str(letter))
            english += MORSE_CODE[letter]
            logging.debug('english so far: ' + str(english))
        english += ' '
        logging.debug('')

    logging.debug('!!! Finished english: ' + str(english))
    logging.debug('----------------------')
    logging.debug('')
